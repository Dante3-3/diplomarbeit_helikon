#!/usr/bin/env python3
"""
Evaluator V3: Evaluates results from results_v3_content.jsonl
Uses 'final_response' field (post-content-review output).
Output: diplomarbeit_output/diplomarbeit_report_v3.md
"""
...
RESULTS_FILE = os.path.join(OUTPUT_DIR, "results_v3_content.jsonl")
REPORT_FILE = os.path.join(OUTPUT_DIR, "diplomarbeit_report_v3.md")
...
def call_copilot(prompt, timeout=120):
    try:
        proc = subprocess.run(["copilot", "-p", prompt, "--yolo"],
...
def evaluate_model_teil(teil_key, teil_name, kategorie, model, outputs, config):
    ...
    for i, o in enumerate(outputs, 1):
        ...
        outputs_text += o.get("final_response", "")[:3000]
        outputs_text += "\n"
...
def generate_report():
    ...
    report_lines = ["# Diplomarbeit V3 – Content Feedback Loop Results\n",
                    f"Generated: {time.strftime('%Y-%m-%d %H:%M:%S')}\n",
                    f"Reviewer model: qwen2.5:7b (content checks)\n",
                    "---\n"]
    ...
    for teil_key in teil_keys:
        ...
        for model, outputs in models.items():
            ...
            report_lines.append(f"\n### Model: {model}\n")
            for o in outputs:
                human_flag = o.get("human_review_required", False)
                report_lines.append(f"\n#### {o['test_id']} – Rep {o['rep']}\n")
                report_lines.append(
                    f"*Structural: {'PASS' if o['v2_structural_pass'] else 'FAIL'} | "
                    f"Content: {'HUMAN REVIEW' if human_flag else ('PASS' if o['content_pass'] else 'FAIL')} | "
                    f"Content-revised: {'Yes' if o['was_content_revised'] else 'No'}*\n"
                )
                if not o["content_pass"] and o.get("content_issues") and not human_flag:
                    report_lines.append(f"**Content issues:** {', '.join(o['content_issues'])}\n")
                report_lines.append(f"\n{o['final_response']}\n\n---\n")
    ...
    struct_fail = sum(1 for r in all_results if not r["v2_structural_pass"])
    content_fail = sum(1 for r in all_results if not r["content_pass"] and not r.get("human_review_required"))
    content_revised = sum(1 for r in all_results if r["was_content_revised"])
    human_rev = sum(1 for r in all_results if r.get("human_review_required"))

    report_lines.append(f"\n## Feedback Loop Statistics\n")
    report_lines.append(f"- Total outputs: {total}\n")
    report_lines.append(f"- V2 structural FAIL rate: {struct_fail}/{total} ({100*struct_fail//max(total,1)}%)\n")
    report_lines.append(f"- V3 content FAIL rate: {content_fail}/{total-human_rev} (excluding human-review)\n")
    report_lines.append(f"- Content-revised: {content_revised}\n")
    report_lines.append(f"- Human review required (German Ortho): {human_rev}\n")
