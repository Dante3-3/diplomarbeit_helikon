...
CONFIG_PATH = "/Users/dstoilovski/Documents/Helikon/diplomarbeit_config_grammar.json"
...
REPORT_FILE = os.path.join(OUTPUT_DIR, "diplomarbeit_report_grammar_split.md")
...
CATEGORY_CRITERIA = {
    ...
    "Grammar Gap-Fill": [
        "Correct and unambiguous answer for every blank",
        "Base form provided in brackets for each gap",
        "Contextual embedding in provided text"
    ],
    "Grammar Error Correction": [
        "Exactly one error per sentence",
        "Error clearly marked in bold",
        "Unambiguous single correct fix"
    ],
    "Grammar Sentence Transformation": [
        "Transformation preserves original meaning",
        "Partial sentence starter provided",
        "Exactly one grammatically correct transformation"
    ],
    "True/False": [
...
def call_copilot(prompt, timeout=120):
    """Call GitHub Copilot CLI for evaluation."""
    try:
        proc = subprocess.run(
