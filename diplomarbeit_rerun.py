#!/usr/bin/env python3
"""
Re-run problematic outputs where models returned clarification questions
instead of actual exercises. Replaces bad entries in results.jsonl.
"""
...
# List of problematic outputs to re-run: (teil, model, test_id, rep)
RERUN_LIST = [
    # teil2: gemma2:9b V1 Rep 3
    ("teil2", "gemma2:9b", "V1", 3),
    # teil3: gemma2:9b MC1 Rep 1-3, MC2 Rep 2-3, MC3 Rep 1,3
    ("teil3", "gemma2:9b", "MC1", 1),
    ("teil3", "gemma2:9b", "MC1", 2),
    ("teil3", "gemma2:9b", "MC1", 3),
    ("teil3", "gemma2:9b", "MC2", 2),
    ("teil3", "gemma2:9b", "MC2", 3),
    ("teil3", "gemma2:9b", "MC3", 1),
    ("teil3", "gemma2:9b", "MC3", 3),
    # teil4: gemma2:9b ALL SC1-SC3 ALL reps
    ...
    # teil6: gemma2:9b ALL TF1-TF3 ALL reps
    ...
    # teil7: gemma2:9b GF1 Rep 1, deepseek-r1:8b GF1 Rep 1, GF2 Rep 1, GF3 Rep 2-3
    ("teil7", "gemma2:9b", "GF1", 1),
    ("teil7", "deepseek-r1:8b", "GF1", 1),
...
def call_ollama(model, system_prompt, user_prompt):
    """Call Ollama API via curl and return the response text."""
    payload = {
        "model": model,
        "messages": [
...
def is_problematic(response):
    """Check if a response is a clarification question instead of an exercise."""
...
def main():
    ...
    print(f"Re-running {len(RERUN_LIST)} problematic outputs...\n")
    ...
    for idx, (teil_key, model, test_id, rep) in enumerate(RERUN_LIST, 1):
        ...
        if is_problematic(response):
            print(f"STILL BAD ({elapsed:.1f}s) - retrying once more...", end="", flush=True)
            start_time = time.time()
            response = call_ollama(model, system_prompt, user_prompt)
            elapsed = time.time() - start_time
        ...
        # Find and replace the matching entry in all_results
        found = False
